...
const BalancesPage = () => {
    ...
    const translateCurriculum = (curriculum: string | null | undefined) => {
        if (!curriculum) return '-';
        switch (curriculum.toLowerCase()) {
            case 'egyptian':
                return t('dashboard.egyptian');
            case 'saudi':
                return t('dashboard.saudi');
            default:
                return curriculum;
        }
    };
    ...
    const filteredUsers = users.filter(user =>
        ...
        (user.curriculum && user.curriculum.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (user.level && user.level.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (user.language && user.language.toLowerCase().includes(searchTerm.toLowerCase())) ||
                ...
                <Card>
                    <CardHeader>
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <Input
                                placeholder={t('dashboard.searchByNamePhoneCurriculumLevelLanguageGrade')}
                                value={searchTerm}
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className={isRTL ? "text-right" : "text-left"}>{t('dashboard.curriculum')}</TableHead>
                                    <TableHead className={isRTL ? "text-right" : "text-left"}>{t('dashboard.level')}</TableHead>
                            ...
                            <TableBody>
                                {studentUsers.map((user) => (
                                    <TableRow key={user.id}>
                                        ...
                                        <TableCell className={isRTL ? "text-right" : "text-left"}>{translateCurriculum(user.curriculum)}</TableCell>
                                        <TableCell className={isRTL ? "text-right" : "text-left"}>{user.level || '-'}</TableCell>
                ...
                <Card>
                    <CardContent className="p-6">
                        <div className="text-center text-muted-foreground">
                            {users.length === 0 ? "No users found" : "No students found"}
                        </div>
                    </CardContent>
                </Card>
            )}
