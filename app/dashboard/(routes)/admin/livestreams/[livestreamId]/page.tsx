...
export default function EditLiveStreamPage({ params }: { params: Promise<{ livestreamId: string }> }) {
    ...
    const fetchData = async () => {
      try {
        const [coursesRes, liveStreamRes] = await Promise.all([
          ...
          fetch(`/api/admin/livestreams/${(await params).livestreamId}`)
        ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/admin/livestreams/${(await params).livestreamId}`, {
        method: "PATCH",
        headers: {
  ...
  const handlePublish = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/livestreams/${liveStream.id}/publish`, {
        method: 'PATCH',
      });
