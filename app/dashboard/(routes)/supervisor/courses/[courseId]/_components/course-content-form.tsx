...
export const CourseContentForm = ({
    ...
    const onEdit = (id: string, type: "chapter" | "quiz") => {
        if (type === "chapter") {
            router.push(`${routeBasePath}/courses/${courseId}/chapters/${id}`);
        } else {
            router.push(`${routeBasePath}/quizzes/${id}/edit`);
        }
    }
    ...
    return (
        <div className="relative mt-6 border bg-card rounded-md p-3 sm:p-4 flex flex-col min-h-[200px]">
            ...
            <div className="flex flex-row gap-2 mt-4 pt-4 border-t flex-wrap">
                <Button 
                    onClick={() => router.push(`${routeBasePath}/quizzes/create?courseId=${courseId}`)} 
                    variant="ghost"
                    size="sm"
